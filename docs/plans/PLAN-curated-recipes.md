...
| ~~_Authors new recipes for azure-cli, ansible, and argocd. Gcloud deferred to #2328 (no version source for Google's distribution channel). Bazel deferred to #2330 (single-binary self-extraction fails in the test-recipe sandbox; needs an installer-script or homebrew approach)._~~ | | |
