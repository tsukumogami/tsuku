...
| ~~_Ships `recipes/a/azure-cli.toml` with explicit `metadata.binaries = ["bin/az"]` and propagates the field through the plan path so the sandbox install creates the right wrapper. The shipped pipx version-constraint mechanism (PR #2352) replaced the manual constraint approach #2345 originally proposed: the resolver auto-picks the newest PyPI release whose `requires_python` matches the bundled python-standalone, so neither recipe needs an explicit pin._~~ | | |
...
graph TD
    ...
    class I2325,I2327,I2328,I2330,I2331,I2333,I2335,I2336,I2338,I2343,I2344,I2345,I2365,I2368,I2370 done
    class I2349 blocked
```
...
- ~~**#2345 (ansible, azure-cli)** is gated by a release containing #2331.~~ Done — both recipes ship in `recipes/a/`, and `metadata.binaries` now propagates through the plan path so sandbox CI can name pipx wrappers correctly.
