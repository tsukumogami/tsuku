...
| ~~_Curates the existing `libnghttp3` recipe (the homebrew formula is `libnghttp3`, not `nghttp3`) and exposes `libnghttp3.9.dylib` for curl, rewrites `utf8proc` from batch garbage (tmux), and updates `pcre2` to install its dylib on macOS (git). The libevent macOS path is deferred to #2333 because tsuku's homebrew bottle resolver does not yet understand revision-suffixed manifests; the libevent recipe keeps its `darwin/*` `unsupported_platforms` entries until the resolver is fixed._~~ | | |
