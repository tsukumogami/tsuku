...
decision: Migrate registry golden files to Cloudflare R2 with CI-generated files on merge, three-tier fallback degradation, and phased 7-phase rollout.
...
1. Git-based fallback cache (~40 MB compressed)
2. Health check before validation starts
3. Three-tier degradation: R2 → Fallback → Skip
4. Automatic issue creation on degraded mode
...
- 9-week phased migration required
...
- **CI Reliability (Critical)**: Three-tier degradation (R2 → Fallback → Skip) ensures validation never blocks development.
- **Graceful Degradation (High)**: Git-based fallback cache provides 7+ days of resilience during R2 outages.
...
5. **9-week migration period**: Phased rollout adds short-term complexity. Mitigated by: each phase is independently reversible; parallel operation during transition.
...
│                           GitHub Repository                              │
│  ┌──────────────┐  ┌──────────────┐  ┌─────────────────────────────────┐│
│  │  recipes/    │  │ embedded/    │  │ .github/cache/golden-fallback/ ││
│  │  (registry)  │  │ (git golden) │  │ (compressed R2 cache)          ││
│  └──────────────┘  └──────────────┘  └─────────────────────────────────┘│
└─────────────────────────────────────────────────────────────────────────┘
                               │
                               ▼
...
│                                                                          │
│  ┌────────────────┐  ┌────────────────┐  ┌────────────────────────────┐ │
│  │ Post-Merge     │  │ Nightly        │  │ Weekly Fallback Sync       │ │
│  │ Generation     │  │ Validation     │  │                            │ │
│  │ (3 platforms)  │  │ (download+test)│  │ (R2 → git cache)          │ │
│  └───────┬────────┘  └───────┬────────┘  └───────────────┬────────────┘ │
│          │                   │                           │              │
└──────────┼───────────────────┼───────────────────────────┼──────────────┘
           │                   │                           │
...
1. Run health check
   │
   ├── Success ──► Download from R2
   │              │
   │              ▼
   │              Run validation against downloaded files
   │
   ├── Slow/Fail ──► Check fallback cache freshness
   │                 │
   │                 ├── Fresh (<7 days) ──► Use fallback
   │                 │
   │                 └── Stale/Missing ──► Skip validation
   │                                       Create GitHub issue
   │
   ▼
2. Report results, create issue on failure
```
...
**Deliverables:**
- `sync-golden-fallback.yml` weekly workflow
- Compressed cache structure in git (tar.zst format)
- Fallback extraction logic
- Cache freshness checks (7-day TTL)
...
- Three-tier degradation logic (R2 → Fallback → Skip)
- Skip validation when R2 unavailable
...
- **Resilient validation**: Three-tier fallback prevents development blocks
...
- **Migration complexity**: 9-week rollout with parallel operation
...
|---------------------|------------|
| R2 dependency | Git-based fallback cache; three-tier degradation |
...
| Cloudflare outage/corruption | Three-tier fallback; git cache provides 7+ days resilience | Complete infrastructure failure (git + R2 + cache) |
