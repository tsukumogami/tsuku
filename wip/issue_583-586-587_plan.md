...
- [ ] 1. Fetch source URLs and checksums from Homebrew API for bash, python@3.13, readline
- [ ] 2. Convert bash-source.toml to primitives (download_file + extract)
- [ ] 3. Convert python-source.toml to primitives
- [ ] 4. Convert readline-source.toml to primitives
- [ ] 5. Update llm-test-matrix.json
- [ ] 6. Run tests to verify fixtures work
- [ ] 7. Delete homebrew_source.go and homebrew_source_test.go
- [ ] 8. Remove homebrew_source from action registry
- [ ] 9. Remove from action_test.go and decomposable_test.go
- [ ] 10. Update validator.go to reject homebrew_source
- [ ] 11. Run tests to verify action removal
- [ ] 12. Remove source build code from homebrew.go
- [ ] 13. Remove source build tests from homebrew_test.go
- [ ] 14. Run full test suite and verify build
...
- [ ] All three test fixtures converted to primitives
- [ ] `homebrew_source` action completely removed
- [ ] HomebrewBuilder source code removed (~1,000+ lines)
- [ ] All tests pass
- [ ] `go build ./...` succeeds
- [ ] `go vet ./...` passes
- [ ] No references to `homebrew_source` remain (except design docs)
