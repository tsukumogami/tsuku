...
summary:
  constraints:
    ...
    - Flag is CLI-only exposure of resolver's requireEmbedded parameter
    - Must work with both eval and install commands
  integration_points:
    - cmd/tsuku/eval.go - add flag and pass to plan generation
    - cmd/tsuku/install.go - add flag and pass through install flow
    - internal/actions/resolver.go - ResolveTransitive already has requireEmbedded param from #1044
  risks:
    - Need to find where ResolveTransitive is called in eval/install flows
    - May need to thread the flag through multiple layers
  approach_notes: |
    Add --require-embedded bool flag to eval and install commands. The flag
    value needs to flow through to ResolveTransitive calls. From #1044, the
    resolver already accepts requireEmbedded parameter - this issue just
    exposes it via CLI.
---

# Implementation Context: Issue #1045
...
graph TD
    ...
    class 1043,1044 done
    class 1045,1046,1047 ready
    class 1048 blocked
```
