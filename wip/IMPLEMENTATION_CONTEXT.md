...
summary:
  constraints:
    - 3-second timeout for all probers
    - EcosystemProber interface lives in builders package
    - Only builders implementing EcosystemProber should be included
  integration_points:
    - cmd/tsuku/create.go runDiscovery() function
    - internal/discover/ecosystem_probe.go NewEcosystemProbe constructor
    - internal/builders package (builder constructors)
  risks:
    - Import cycle if discover imports builders constructors directly from cmd (unlikely, cmd already imports both)
    - Builder constructors may require non-nil HTTP clients for production use
  approach_notes: |
    Construct all 7 ecosystem builders in runDiscovery(), type-assert to EcosystemProber,
    pass to NewEcosystemProbe with 3s timeout. Replace the zero-value stub.
---

# Implementation Context: Issue #1385
