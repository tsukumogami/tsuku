...
summary:
  constraints:
    - llama_tokenize returns negative on error - must handle buffer sizing correctly
    - Model uses Qwen 2.5 tokenizer which may have specific encoding requirements
    - Tokenizer must handle UTF-8 input properly
  integration_points:
    - tsuku-llm/src/llama/context.rs - tokenization wrapper
    - tsuku-llm/src/main.rs - Complete RPC handler
    - internal/llm/lifecycle_integration_test.go - test to unskip
  risks:
    - Buffer size calculation may be incorrect
    - Special tokens (BOS/EOS) handling may differ between models
    - Input encoding mismatch between Go UTF-8 and llama.cpp expectations
  approach_notes: |
    The tokenization error "llama_tokenize returned negative count" indicates
    the llama.cpp tokenize function is failing. Common causes:
    1. Buffer too small - need to query token count first, then allocate
    2. Special tokens flag misconfigured
    3. Model tokenizer not properly initialized
    4. Input encoding issues

    Need to examine the tokenization code in llama/context.rs and fix the
    root cause.
---

# Implementation Context: Issue #1676
...
                                                                                                                                                                                                                                                                                                       Context Extraction Warnings:
  - Agent table parsing not implemented - using regex fallback
  - Regex parsing failed - using full design doc

{
  "status": "degraded",
  "context_available": true,
  "context_file": "wip/IMPLEMENTATION_CONTEXT.md",
  "tier": "testable",
  "warnings": [
    "Agent table parsing not implemented - using regex fallback",
    "Regex parsing failed - using full design doc"
