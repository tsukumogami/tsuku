...
summary:
  constraints:
    - Action dependencies must use RequireEmbedded flag when set
    - Recipe-level dependencies must ALWAYS use RequireEmbedded=false (normal loading)
    - Transitive dependencies of action deps must inherit RequireEmbedded flag
    - Platform-specific deps (LinuxInstallTime, DarwinInstallTime) need same handling
  integration_points:
    - internal/actions/resolver.go - ResolveTransitive functions need RequireEmbedded propagation
    - RecipeLoader interface already accepts LoaderOptions from #1043
    - plan_generator.go calls ResolveTransitive - may need updates
  risks:
    - Must distinguish action deps (require embedded) from recipe-level deps (normal loading)
    - Transitive resolution uses visited map - ensure flag propagates correctly
    - Test coverage for all code paths (action deps, recipe deps, transitive)
  approach_notes: |
    Add ResolveOptions struct with RequireEmbedded bool. Update resolveTransitiveSet
    to pass LoaderOptions{RequireEmbedded: opts.RequireEmbedded} when loading action
    dependencies. The key is that loader.GetWithContext already accepts LoaderOptions
    from #1043 - this issue is about ensuring the resolver passes the correct value
    based on dependency source.
---

# Implementation Context: Issue #1044

**Source**: docs/designs/DESIGN-embedded-recipe-list.md (None)
...
graph TD
    ...
    class 1043 done
    class 1044,1046,1047 ready
    class 1045,1048 blocked
```
