...
summary:
  constraints:
    - Walking skeleton - establishes gRPC contract and interfaces that all other issues refine
    - ServerLifecycle uses lock file protocol at $TSUKU_HOME/llm.sock.lock for reliable daemon detection
    - LocalProvider implements existing Provider interface - no changes to interface itself
    - Factory registers LocalProvider as lowest priority fallback (after Claude and Gemini)
    - AddonManager is a stub in this issue - full download logic comes in #1629
  integration_points:
    - internal/llm/local.go - LocalProvider implementing Provider interface
    - internal/llm/lifecycle.go - ServerLifecycle with EnsureRunning, IsRunning, Stop
    - internal/llm/addon.go - AddonManager stub with AddonPath, IsInstalled
    - internal/llm/factory.go - NewFactory() to register LocalProvider when local_enabled=true
    - proto/llm.proto - gRPC contract for InferenceService
  risks:
    - Prototype code exists - need to refine to match design, not add features beyond skeleton
    - Lock file implementation must handle cross-platform differences (Unix domain sockets)
    - Integration test requires mock gRPC server - keep it minimal for skeleton
  approach_notes: |
    This is the walking skeleton issue. The goal is to establish the foundational types
    and interfaces that subsequent issues will flesh out. The prototype already exists
    on this branch - the task is to refine it to match the design document exactly,
    removing any features beyond the skeleton scope.

    Key deliverables:
    1. ServerLifecycle with lock file protocol (non-blocking exclusive lock)
    2. LocalProvider that uses ServerLifecycle in Complete()
    3. AddonManager stub (just AddonPath() and IsInstalled())
    4. Factory integration when local_enabled=true
    5. Unit tests for lifecycle manager
    6. Integration test with mock gRPC server
---

# Implementation Context: Issue #1628

**Source**: docs/designs/DESIGN-local-llm-runtime.md (None)
