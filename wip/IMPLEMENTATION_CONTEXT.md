...
summary:
  constraints:
    - 3-second shared timeout via context.WithTimeout
    - Buffered channel sized to len(probers) to prevent goroutine leaks
    - Static priority ranking (cask=1..cpan=7), not dynamic
    - ProbeResult and EcosystemProber live in builders package (moved in #1383)
  integration_points:
    - internal/discover/ecosystem_probe.go (replace stub)
    - internal/builders/probe.go (EcosystemProber interface, ProbeResult)
    - internal/discover/chain.go (DiscoveryResult, Confidence field)
  risks:
    - Name matching logic must handle case sensitivity correctly
    - All-failures case must return error, not (nil, nil)
  approach_notes: |
    Replace stub with NewEcosystemProbe constructor + parallel Resolve().
    goroutine-per-builder fan-out, buffered channel collection, priority sort.
---

# Implementation Context: Issue #1384
