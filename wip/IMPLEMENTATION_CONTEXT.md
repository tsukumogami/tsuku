...
summary:
  constraints:
    - Each Probe() must reuse existing fetch method, no new HTTP requests
    - Soft error handling: API failures return ProbeResult{Exists: false}, not errors
    - Downloads always 0; Age only for Go builder
    - No changes to SessionBuilder interface
    - Each Probe() under 20 lines
  integration_points:
    - internal/discover/ecosystem_probe.go (EcosystemProber interface, ProbeResult struct)
    - internal/builders/{cargo,pypi,npm,gem,go,cpan,cask}.go
  risks:
    - Existing fetch methods may have signatures that don't match expected pattern
    - Go builder Time field parsing format may differ from RFC3339
  approach_notes: |
    Thin wrapper per builder: call existing fetch, map result to ProbeResult.
    Go builder adds time.Parse for Age. All others: Exists + Source only.
---

# Implementation Context: Issue #1383

**Source**: docs/designs/DESIGN-ecosystem-probe.md (None)
