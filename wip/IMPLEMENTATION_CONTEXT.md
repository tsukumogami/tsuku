...
summary:
  constraints:
    - Output format must match design spec (category labels: tsuku, external, system)
    - Error handling must use ExitVerifyFailed exit code
    - Must handle static binaries (no dependencies) gracefully
  integration_points:
    - cmd/tsuku/verify.go - verifyTool() and verifyLibrary() functions
    - verify.ValidateDependencies() from #989 for recursive validation
    - verify.DepResult struct for categorized results
    - install.State for current installation state
  risks:
    - Recipe loader integration may need attention (nil loader in initial impl)
    - Output formatting must work for all category combinations
    - Need to handle case where ValidateDependencies returns empty results
  approach_notes: |
    1. Add displayDependencyResults() helper function for consistent output
    2. Update verifyTool() to call ValidateDependencies after Tier 1 checks
    3. Update verifyLibrary() to call ValidateDependencies after header validation
    4. Handle static binaries by checking for empty dependency results
    5. Follow existing error handling patterns with ExitVerifyFailed
---

# Implementation Context: Issue #990
...
graph TD
    ...
    class I978,I979,I980,I981,I982,I983,I984,I985,I986 done
    class I989 ready
    class I990,I991 blocked
```
