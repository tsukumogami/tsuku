...
summary:
  constraints:
    - Preflight runs as a workflow step (not a separate job) in batch-generate.yml
    - Must read batch-control.json for circuit breaker state and budget
    - Rate limits are per-ecosystem, configured in batch-control.json or hardcoded
    - Circuit breaker field names in batch-control.json use "failures" not "consecutive_failures"
  integration_points:
    - .github/workflows/batch-generate.yml - add preflight step before generation
    - internal/batch/orchestrator.go - add circuit breaker check and rate limiting
    - batch-control.json - read circuit breaker and budget state
    - data/priority-queue.json - read pending packages
  risks:
    - Issue spec mentions Go structs and orchestrator changes but current pipeline is shell-based
    - batch-control.json schema in issue differs from actual file (consecutive_failures vs failures)
    - Rate limiting in workflow shell loop vs Go orchestrator needs clarification
  approach_notes: |
    The issue mixes two concerns: workflow-level preflight (circuit breaker check before
    generation starts) and Go-level rate limiting in the orchestrator. The workflow already
    has generation logic in shell; preflight should be a new step that reads batch-control.json
    and exits early if breaker is open. Rate limiting adds sleep between packages in the
    generation loop.
---

# Implementation Context: Issue #1252

**Source**: docs/designs/DESIGN-batch-recipe-generation.md (None)
