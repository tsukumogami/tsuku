...
summary:
  constraints:
    - Shell-first, no Go code changes
    - batch_id format must match Go orchestrator ({date}-{ecosystem})
    - UTC dates only (-u flag)
    - Trailers must be findable by scripts/rollback-batch.sh via git grep
  integration_points:
    - .github/workflows/batch-generate.yml (merge job)
    - scripts/rollback-batch.sh (consumer of batch_id trailer)
    - INCLUDED_COUNT / EXCLUDED_COUNT variables (already exist in merge job)
  risks:
    - Sequence number derivation depends on git log --grep working across all refs
    - success_rate calculation needs bash arithmetic for two decimal places
  approach_notes: |
    Add a "Generate batch ID" step before the commit step in the merge job.
    Modify the existing commit message construction to include trailers.
    BATCH_ID exported via GITHUB_ENV for downstream #1351.
---

# Implementation Context: Issue #1349

**Source**: docs/designs/DESIGN-merge-job-completion.md (None)
