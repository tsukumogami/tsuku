...
summary:
  constraints:
    - Checksums must use SHA256 (hex-encoded), matching tool binary pattern
    - Symlinks must be skipped during checksum computation (only real files checksummed)
    - Checksum errors should warn but not fail installation (matching tool behavior)
    - Must use relative paths as map keys (e.g., lib/libstdc++.so.6.0.33)
    - LibraryVersionState.Checksums field exists from issue #942 (merged in PR #958)
  integration_points:
    - internal/install/state.go - LibraryVersionState struct (Checksums field already added by #942)
    - internal/install/state_lib.go - need SetLibraryChecksums method
    - internal/install/library.go - InstallLibrary() must call checksum computation
    - internal/install/checksum.go - existing ComputeFileChecksum() can be reused
  risks:
    - Large libraries with many files could slow installation (log warning on error, don't fail)
    - Need to handle symlinks properly (skip them for checksums, only checksum real files)
    - Path handling must be consistent across platforms (use filepath.Clean, relative paths)
  approach_notes: |
    1. Add SetLibraryChecksums(name, version, checksums) to StateManager in state_lib.go
    2. Create ComputeLibraryChecksums(libDir) in checksum.go that walks directory, skips symlinks
    3. Modify InstallLibrary() to call checksum computation after copyDir(), store via SetLibraryChecksums()
    4. Reuse existing ComputeFileChecksum() for individual files
    5. Follow existing pattern from ComputeBinaryChecksums() but walk full directory instead of binary list
---

# Implementation Context: Issue #946

**Source**: docs/designs/DESIGN-library-verification.md (None)
