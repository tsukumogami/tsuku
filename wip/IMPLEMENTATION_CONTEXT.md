...
summary:
  constraints:
    - 11 target environments (5 linux x86_64 + 4 linux arm64 + 2 macOS); archlinux has no arm64 image
    - CGO_ENABLED=0 cross-compilation for 4 platform binaries
    - Per-recipe timeout 5min, per-job timeout 120min
    - Exit code 5 retries with exponential backoff 2s/4s/8s, max 3 attempts
    - macOS needs gtimeout (brew install coreutils) instead of timeout
    - JSON result format: recipe, platform, status, exit_code, attempts
  integration_points:
    - .github/workflows/batch-generate.yml (main file to modify)
    - Platform ID format: {os}-{family}-{libc}-{arch} for Linux, {os}-{arch} for macOS
    - Artifacts: passing-recipes, tsuku-binaries, validation-results-<platform>.json
    - Downstream: #1323 merge job consumes all result artifacts
  risks:
    - ARM64 runner ubuntu-24.04-arm availability
    - macOS CI budget (~110 min per 25-recipe batch)
    - Docker image pull failures on arm64 runners
  approach_notes: |
    Split existing generate job to upload artifacts instead of creating PR.
    Add 4 validation jobs following platform-integration.yml container patterns.
    Linux jobs run family containers sequentially; macOS jobs run natively.
    Fix known bugs: gtimeout on macOS, no archlinux arm64, backoff loop bounds.
---

# Implementation Context: Issue #1320

**Source**: docs/designs/DESIGN-batch-platform-validation.md (None)
