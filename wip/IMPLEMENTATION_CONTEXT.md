...
summary:
  constraints:
    - Addon manifest must be embedded in tsuku (not downloaded)
    - SHA256 verification required at download time AND before each execution
    - Platform detection needed (darwin-arm64, darwin-amd64, linux-amd64, etc.)
    - Store addon at $TSUKU_HOME/tools/tsuku-llm/<version>/tsuku-llm
    - Failed verification must return clear error, not panic
    - Must not break existing E2E skeleton flow
  integration_points:
    - internal/llm/addon/manager.go - expand existing stub with AddonManager struct
    - internal/llm/lifecycle.go - ServerLifecycle.EnsureRunning calls AddonManager.EnsureAddon
    - internal/llm/local.go - LocalProvider.Complete calls EnsureAddon before inference
    - Manifest embedded in binary - likely internal/llm/addon/manifest.go with //go:embed
  risks:
    - No real addon binary exists yet (CI pipeline issue #1633 not done)
    - Need mock/stub manifest for testing until real binaries available
    - Platform detection logic needs careful testing across OS/arch
    - Checksum verification timing (download vs execution) must be clear
  approach_notes: |
    Build on existing addon/manager.go stub which has AddonPath() and IsInstalled().
    Add AddonManager struct with EnsureAddon() that:
    1. Checks if addon exists at expected path
    2. If not, downloads platform-specific binary from manifest URL
    3. Verifies SHA256 at download time
    4. Before each execution (in EnsureRunning), re-verify SHA256

    Manifest structure should include: version, platforms map with URL and checksum.
    Platform key format: GOOS-GOARCH (e.g., darwin-arm64, linux-amd64)

    For now, manifest can have placeholder URLs/checksums since CI pipeline
    doesn't exist yet. Tests should use mock server or file injection.
---

# Implementation Context: Issue #1629
