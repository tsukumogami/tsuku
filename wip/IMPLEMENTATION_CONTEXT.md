...
summary:
  constraints:
    - This issue only adds LoaderOptions to loader - resolver integration is #1044
    - Existing callers must use LoaderOptions{} for default behavior (backwards compatibility)
    - RequireEmbedded=true only checks embedded FS, no local/registry fallback
  integration_points:
    - internal/recipe/loader.go - main changes (LoaderOptions struct, Get/GetWithContext methods)
    - All callers of loader.Get/GetWithContext - must pass LoaderOptions{}
    - internal/recipe/loader_test.go - tests for new behavior
  risks:
    - API signature change affects many callers - need to update all call sites
    - Error message format should be actionable but not leak implementation details
  approach_notes: |
    Focus on loader changes only. Add LoaderOptions struct with RequireEmbedded field.
    Modify Get and GetWithContext to accept options. When RequireEmbedded=true, only
    call loadFromEmbedded and return clear error if not found. Update all existing
    callers to pass LoaderOptions{} for default behavior.
---

# Implementation Context: Issue #1043
