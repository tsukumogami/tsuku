...
summary:
  constraints:
    - "--deps-only must be mutually exclusive with --metadata-only"
    - "Platform filtering must happen BEFORE extracting deps (use ResolveTransitiveForPlatform)"
    - "Text output: one package per line for shell consumption"
    - "JSON output must include 'packages' array and 'family' field"
    - "Must work with --recipe flag for local recipe files"
  integration_points:
    - "cmd/tsuku/info.go - extend with new flags"
    - "internal/executor/system_deps.go - new shared extraction library"
    - "internal/actions/resolver.go - uses existing ResolveTransitiveForPlatform()"
    - "internal/executor/filter.go - uses existing FilterStepsByTarget()"
    - "internal/sandbox/packages.go - should consume shared extraction"
  risks:
    - "Existing deps.go prototype in PR #1572 may conflict - need to remove/replace"
    - "Platform target construction from --family flag needs correct libc mapping"
    - "Transitive resolution with platform filtering is complex - edge cases possible"
  approach_notes: |
    1. Create internal/executor/system_deps.go with extraction functions
    2. Add three flags to info.go: --deps-only, --system, --family
    3. Build target from --family (alpine→musl, debian→glibc, etc)
    4. Use ResolveTransitiveForPlatform() for platform-aware resolution
    5. Extract system packages from each recipe in resolved tree
    6. Deduplicate and output (text or JSON based on --json flag)
---

# Implementation Context: Issue #1573

**Source**: docs/designs/DESIGN-recipe-driven-ci-testing.md (None)
