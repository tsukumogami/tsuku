...
func (a *NpmExecAction) executePackageInstall(ctx *ExecutionContext, params map[string]interface{}) error {
	...
	// Do not write the lockfile to the execution directory.
	// The lockfile was generated by the evaluating npm version (e.g., npm 11 on glibc).
	// Writing it causes failures on platforms with older npm (e.g., Alpine apk npm 10)
	// when the lockfileVersion is higher than what the installed npm supports.
	// Since we install an exact version (package@version), the lockfile is not
	// needed for reproducibility — npm resolves the exact version from the command line.
	_ = packageLock // captured at eval time for future use; not written at execution time
