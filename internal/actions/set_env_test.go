...
// TestSetEnvMigratesDataRootOnExport covers the users who installed nvm back when its
// exported root was the versioned tool directory.
//
// The point of running the migration from inside set_env is that the data and the
// pointer change in one event. This asserts both halves of that event together: after
// the step, the fragment names the stable root and the data is in it.
func TestSetEnvMigratesDataRootOnExport(t *testing.T) {
	home := t.TempDir()
	toolsDir := filepath.Join(home, "tools")
	installDir := filepath.Join(toolsDir, "nvm-0.40.6")
	if err := os.MkdirAll(installDir, 0o755); err != nil {
		t.Fatalf("MkdirAll: %v", err)
	}

	// A Node version installed under the previous nvm, in the directory that version's
	// export named.
	legacy := filepath.Join(toolsDir, "nvm-0.40.3", "versions", "node", "v20.0.0", "bin")
	if err := os.MkdirAll(legacy, 0o755); err != nil {
		t.Fatalf("MkdirAll: %v", err)
	}
	if err := os.WriteFile(filepath.Join(legacy, "node"), []byte("node"), 0o755); err != nil {
	...
	ctx := &ExecutionContext{
		...
		Version:        "0.40.6",
		Recipe:         &recipe.Recipe{Metadata: recipe.MetadataSection{Name: "nvm"}},
	}

	action := &SetEnvAction{}
	if err := action.Execute(ctx, map[string]interface{}{
		"vars": []interface{}{
			map[string]interface{}{"name": "NVM_DIR", "value": "{data_dir}"},
		},
	}); err != nil {
		t.Fatalf("Execute() error = %v", err)
	}

	dataRoot := filepath.Join(home, "data", "nvm")

	fragment, err := os.ReadFile(filepath.Join(home, "share", "shell.d", "00-env-nvm@0.40.6.bash"))
	if err != nil {
		t.Fatalf("reading the exported fragment: %v", err)
	}
	if !strings.Contains(string(fragment), dataRoot) {
		t.Errorf("fragment does not export the data root:\n%s", fragment)
	}

	if _, err := os.Stat(filepath.Join(dataRoot, "versions", "node", "v20.0.0", "bin", "node")); err != nil {
		t.Errorf("the previous version's Node install did not follow the export: %v", err)
	}
	if _, err := os.Stat(filepath.Join(legacy, "node")); !os.IsNotExist(err) {
		t.Error("the old copy is still present; the data now exists in two places")
	}
}

// TestSetEnvLeavesUnrelatedExportsAlone pins that the migration is keyed on the variable
// being a data root, not on set_env running at all.
func TestSetEnvLeavesUnrelatedExportsAlone(t *testing.T) {
	home := t.TempDir()
	toolsDir := filepath.Join(home, "tools")
	installDir := filepath.Join(toolsDir, "nvm-0.40.6")
	if err := os.MkdirAll(installDir, 0o755); err != nil {
	...
	legacy := filepath.Join(toolsDir, "nvm-0.40.3", "versions", "node", "v20.0.0")
	if err := os.MkdirAll(legacy, 0o755); err != nil {
		t.Fatalf("MkdirAll: %v", err)
	}
	...
	if err := action.Execute(ctx, map[string]interface{}{
		"vars": []interface{}{
			map[string]interface{}{"name": "SOMETHING_ELSE", "value": "{install_dir}"},
		},
	}); err != nil {
		t.Fatalf("Execute() error = %v", err)
	}
	...
	if _, err := os.Stat(legacy); err != nil {
		t.Errorf("data was moved by a step that exported no data root: %v", err)
	}
}
