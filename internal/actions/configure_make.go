...
func (a *ConfigureMakeAction) Execute(ctx *ExecutionContext, params map[string]interface{}) error {
	...
	reporter.Log("   Prefix: %s", prefix)
	reporter.Log("   Executables: %v", executables)
	if len(configureArgs) > 0 {
		reporter.Log("   Configure args: %v", configureArgs)
	}
	...
	if !skipConfigure {
		reporter.Log("   Running: ./configure --prefix=%s", prefix)
		...
		reporter.Log("   Skipping ./configure (skip_configure=true)")
		reporter.Log("   Building with make directly using make_args")
	...
	for _, target := range makeTargets {
		...
		if target != "" {
			...
			reporter.Log("   Running: make %s", target)
		} else {
			reporter.Log("   Running: make")
		}
