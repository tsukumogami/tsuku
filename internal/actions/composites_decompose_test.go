...
func TestHashiCorpReleaseAction_Decompose(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			checkDownload: func(t *testing.T, params map[string]interface{}) {
				...
				if dest != "terraform_1.5.0_linux_amd64.zip" {
					t.Errorf("dest = %q, want zip filename", dest)
				}
			},
			checkBinary: func(t *testing.T, steps []Step) {
				// Binary defaults to product name
				chmodFiles := steps[2].Params["files"].([]string)
				if len(chmodFiles) != 1 || chmodFiles[0] != "terraform" {
					t.Errorf("chmod files = %v, want [terraform]", chmodFiles)
				}
			},
		},
		{
			name: "vault with custom binary name",
			params: map[string]interface{}{
				"product": "vault",
				"binary":  "vault-bin",
			},
			...
			checkBinary: func(t *testing.T, steps []Step) {
				chmodFiles := steps[2].Params["files"].([]string)
				if len(chmodFiles) != 1 || chmodFiles[0] != "vault-bin" {
					t.Errorf("chmod files = %v, want [vault-bin]", chmodFiles)
				}
				binaries := steps[3].Params["binaries"].([]interface{})
				if len(binaries) != 1 || binaries[0] != "vault-bin" {
					t.Errorf("binaries = %v, want [vault-bin]", binaries)
				}
			},
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			steps, err := action.Decompose(testCtx, tt.params)
			if (err != nil) != tt.wantErr {
				t.Errorf("Decompose() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			...
			if tt.checkBinary != nil {
				tt.checkBinary(t, steps)
			}
		})
	}
}
