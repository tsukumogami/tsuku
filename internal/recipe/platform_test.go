...
func TestValidateStepsAgainstPlatforms(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			supportedOS:   []string{"linux", "darwin"},
			supportedArch: nil, // Defaults to tsuku-supported arch (amd64, arm64)
			steps: []Step{
				{
					Action: "require_system",
					Params: map[string]interface{}{
			...
			expectedErrors:  2, // linux/amd64 and linux/arm64 not covered
			errorSubstrings: []string{"install_guide missing coverage for platform 'linux/amd64'", "install_guide missing coverage for platform 'linux/arm64'"},
			description:     "install_guide with missing OS should fail for all platforms of that OS",
		},
		{
		...
		},
		{
			...
			expectedErrors:  4, // os_mapping error, arch_mapping error, invalid OS key 'linux', install_guide missing darwin/amd64
			errorSubstrings: []string{"os_mapping contains 'linux'", "arch_mapping contains 'arm64'", "install_guide contains OS key 'linux'", "install_guide missing coverage for platform 'darwin/amd64'"},
		...
		// Platform tuple tests
		{
			name:          "install_guide with platform tuples (exact coverage)",
			supportedOS:   []string{"linux", "darwin"},
			supportedArch: []string{"amd64", "arm64"},
			steps: []Step{
				{
					...
					Params: map[string]interface{}{
						"command": "docker",
						"install_guide": map[string]interface{}{
							"linux/amd64":  "apt install docker (Intel)",
							"linux/arm64":  "apt install docker (ARM)",
			...
			expectedErrors: 0,
			description:    "install_guide with exact platform tuple coverage should pass",
		},
		{
			name:          "install_guide with OS fallback",
			supportedOS:   []string{"linux", "darwin"},
			supportedArch: []string{"amd64", "arm64"},
			steps: []Step{
				{
			...
			expectedErrors: 0,
			description:    "install_guide with OS-only keys should provide coverage for all arch variants",
		},
		{
			name:          "install_guide with generic fallback",
			supportedOS:   []string{"linux", "darwin"},
			supportedArch: []string{"amd64", "arm64"},
			steps: []Step{
				{
					...
					Params: map[string]interface{}{
						"command": "docker",
						"install_guide": map[string]interface{}{
							"fallback": "see https://docker.com/install",
						},
					},
				},
			},
			expectedErrors: 0,
			description:    "install_guide with only fallback key should provide coverage for all platforms",
		},
		{
			name:          "install_guide with mixed keys (tuple + OS)",
			supportedOS:   []string{"linux", "darwin"},
			supportedArch: []string{"amd64", "arm64"},
			steps: []Step{
				{
					...
					Params: map[string]interface{}{
						"command": "docker",
						"install_guide": map[string]interface{}{
							"darwin/arm64": "brew install docker (ARM-specific)",
							"darwin":       "brew install docker",
							"linux":        "apt install docker",
						},
					},
			...
			expectedErrors: 0,
			description:    "install_guide with mixed tuple and OS keys should pass",
		},
		{
			name:          "install_guide with invalid platform tuple",
			supportedOS:   []string{"linux"},
			supportedArch: []string{"amd64"},
			steps: []Step{
				{
			...
			expectedErrors:  1,
			errorSubstrings: []string{"install_guide contains platform tuple 'darwin/arm64'", "not in the recipe's supported platforms"},
			description:     "install_guide with tuple not in supported platforms should fail",
		},
		{
			name:          "install_guide with invalid OS key",
			supportedOS:   []string{"linux"},
			supportedArch: []string{"amd64"},
			steps: []Step{
				{
			...
			expectedErrors:  1,
			errorSubstrings: []string{"install_guide contains OS key 'darwin'", "not in the recipe's supported platforms"},
			description:     "install_guide with OS key not in supported platforms should fail",
		},
		{
			name:          "install_guide with missing platform coverage",
			supportedOS:   []string{"linux", "darwin"},
			supportedArch: []string{"amd64", "arm64"},
			steps: []Step{
				{
					...
					Params: map[string]interface{}{
						...
						"install_guide": map[string]interface{}{
							"linux/amd64":  "apt install docker (Intel)",
							"darwin/amd64": "brew install docker (Intel)",
							// Missing: linux/arm64, darwin/arm64
						},
					},
				},
			},
			expectedErrors:  2,
			errorSubstrings: []string{"install_guide missing coverage for platform 'linux/arm64'", "install_guide missing coverage for platform 'darwin/arm64'"},
			description:     "install_guide with incomplete platform coverage should fail",
		},
		{
			name:          "install_guide with partial tuple and fallback",
			supportedOS:   []string{"linux", "darwin"},
			supportedArch: []string{"amd64", "arm64"},
			steps: []Step{
				{
			...
			expectedErrors: 0,
			description:    "install_guide with some tuples and fallback should cover all platforms",
		},
	}
